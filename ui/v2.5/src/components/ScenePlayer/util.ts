...
export function addPseudoFullscreenListener(listener: (isFullscreen: boolean) => void): () => void {
  pseudoFullscreenListeners.push(listener);
  return () => {
    pseudoFullscreenListeners = pseudoFullscreenListeners.filter(l => l !== listener);
  };
}
